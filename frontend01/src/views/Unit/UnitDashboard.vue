...
import Unit from '../SuperAdmin/pages/Unit.vue';
  export default {
    setup() {
      ...
      const itemsPerPage = 20; // Number of items per page for pagination
...
// Insert the logo
doc.addImage(logo, 'PNG', 18, 22, 15, 15); // Adjust the x, y, width, and height as needed
...
const pageWidth = doc.internal.pageSize.getWidth();
    ...
    // Header
    centerText('Province of Bukidnon', yPos, 16);
    yPos += 10;
    centerText('City of Malaybalay', yPos, 16);
    yPos += 10;
    centerText('Market Site Brgy 9, Malaybalay City Bukidnon', yPos, 12);
    yPos += 10;
    centerText('City Economic Enterprise Development and Management Office', yPos, 12);
    yPos += 10;
    centerText('Official Receipt', yPos, 16);
    yPos += 10;
    centerText(branchHeader, yPos, 12);
    yPos += 20;
    ...
    // Receipt Content
    doc.setFontSize(12);
    doc.text(`Branch: ${transaction.branch}`, 20, yPos);
    yPos += 10;
    doc.text(`Date: ${formattedDate}`, 20, yPos);
    yPos += 10;
    doc.text(`Teller: ${transaction.teller}`, 20, yPos);
    yPos += 10;
    doc.text(`Amount: PHP ${transaction.amount.toFixed(2)}`, 20, yPos);
    yPos += 10;
    doc.text(`Type: ${transaction.type}`, 20, yPos);
    yPos += 10;
    doc.text(`Reference: ${transaction.reference_key}`, 20, yPos);
    yPos += 10;
    doc.text(`Unit : ${unit.unit_info}`, 20, yPos);
    yPos += 10;
    doc.text(`Unit Type : ${unit.unit_type}`, 20, yPos);
    yPos += 10;
    ...
    // Footer or Remarks
    yPos += 20;
    centerText('Thank you for your payment!', yPos, 12);
    yPos += 10;
    centerText('For any inquiries, please contact us at (123) 456-7890.', yPos, 10);
    yPos += 10;
    centerText('This is a computer-generated receipt and does not require a signature.', yPos, 10);
  ...
  max-width: 1300px; /* Limit the maximum width of the modal */
