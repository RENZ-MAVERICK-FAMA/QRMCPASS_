<template>
  ...
  export default {
    ...
    methods: {
    loginkiosk() {
      axios.post('http://127.0.0.1:9000/loginkiosk', this.formData)
        .then(response => {
          // Store the access token in local storage
          localStorage.setItem('access_token', response.data.access_token);
          localStorage.setItem('accountType', response.data.accountType);
          // Redirect to the home page or perform any other action
          if (localStorage.getItem('access_token') && localStorage.getItem('accountType') === 'kiosk') {
            
            this.$router.push('/kioskhome');
  ...
  created() {
  const isAuthenticated = localStorage.getItem('access_token') && localStorage.getItem('accountType') === 'kiosk';
  if (isAuthenticated) {
    this.$emit('show', true);
