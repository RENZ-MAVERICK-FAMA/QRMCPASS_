...
localStorage.setItem('detectedCodesMotorela', JSON.stringify(this.detectedCodes));
      ...
      axios.get('http://127.0.0.1:9000/delinquencies/motorela', {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('access_token')}`,
        },
      })
    ...
    this.clearLocalStorageInterval = setInterval(() => {
      ...
      if (now.getHours() === 0 && now.getMinutes() === 0) {
        localStorage.removeItem('detectedCodesMotorela');
        this.detectedCodesMotorela = []; // Optionally, clear the detectedCodes array in the component
      }
...
const filename = `Motorela booth qrcodes and delinquencies report for ${currentDate}.xlsx`;
// Prepare data for Excel
const data = [
