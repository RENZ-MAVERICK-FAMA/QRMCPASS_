<template>
  ...
  export default {
    ...
    methods: {
    loginunit() {
      axios.post('https://k1dobevofh.ap.loclx.io/loginunit', this.formData)
        .then(response => {
          // Store the access token in local storage
          localStorage.setItem('access_token', response.data.access_token);
          localStorage.setItem('accountType', response.data.accountType);
          // Redirect to the home page or perform any other action
          if (localStorage.getItem('access_token') && localStorage.getItem('accountType') === 'unit') {
  ...
  created() {
  const isAuthenticated = localStorage.getItem('access_token') && localStorage.getItem('accountType') === 'unit';
  if (isAuthenticated) {
    this.$emit('show', true);
