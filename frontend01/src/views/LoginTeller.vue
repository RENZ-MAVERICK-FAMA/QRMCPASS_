<template>
    ...
    login() {
      axios.post('http://127.0.0.1:9000/loginTeller', this.formData)
        .then(response => {
          ...
          if (localStorage.getItem('access_token') && localStorage.getItem('accountType') === 'teller') {
  ...
  created() {
  const isAuthenticated = localStorage.getItem('access_token') && localStorage.getItem('accountType') === 'teller';
  if (isAuthenticated) {
    this.$emit('show', true);
