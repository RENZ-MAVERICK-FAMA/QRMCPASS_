<template>
  ...
  import Unit from '../views/Unit.vue';
    export default {
      setup() {
        ...
        const unit = ref({ id: null, unit_info: '', qrcode: '' ,picture: ''});
        const balance = ref(null);
        const showModalDelinquencies = ref(false);
        const showModalTransactions = ref(false);
        const delinquencies = ref([]);
        const transactions = ref([]);
        const currentPageDelinquencies = ref(0);
        const currentPageTransactions = ref(0);
        const itemsPerPage = 5; // Number of items per page for pagination
        const currentDate = ref(new Date()); // Current date for calendar
        const unitsData = ref([]); // Units data fetched from the API
        const router = useRouter();
        const updateInterval = ref(null);  // Store interval ID
      const isMounted = ref(false);
        

        // Computed properties for the current month and year
        ...
        const fetchUnitsData = async () => {
          try {
            const response = await axios.get('https://k1dobevofh.ap.loclx.io/paid', {
              headers: {
        ...
        const fetchBalances = async (unitId) => {
          try {
            const response = await axios.get(`https://k1dobevofh.ap.loclx.io/unit/${unitId}/balances`, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('access_token')}`
              }
            });
        ...
        const fetchDelinquencies = async (unitId) => {
          try {
            const response = await axios.get(`https://k1dobevofh.ap.loclx.io/unit/${unitId}/delinquencies`, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('access_token')}`
              }
            });
            delinquencies.value = response.data.delinquencies;
          } catch (error) {
            console.error('Error fetching delinquencies for unit:', unitId, error);
          }
        };
        ...
        const fetchTransactions = async (unitId) => {
          try {
            const response = await axios.get(`https://k1dobevofh.ap.loclx.io/unit/${unitId}/transactions`, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('access_token')}`
              }
            });
            transactions.value = response.data.transactions;
            console.log(transactions.value = response.data.transactions);
          } catch (error) { 
            console.error('Error fetching transactions for unit:', unitId, error);
          }
        };
        ...
        const fetchUnitDetails = async () => {
          try {
            const response = await axios.get('https://k1dobevofh.ap.loclx.io/unitdetails', {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('access_token')}`
              }
            });
            unit.value = response.data;
            console.log(unit.value = response.data);
            await fetchBalances(unit.value.id);
            await fetchDelinquencies(unit.value.id);
            await fetchTransactions(unit.value.id);
          } catch (error) {
            console.error('Error fetching unit details:', error);
          }
        };

        // Fetch initial unit details
        fetchUnitDetails();

        // Watch for changes in currentDate and fetch new data if it changes
        watch(currentDate, fetchUnitsData);

        // Interval to periodically fetch balances and transactions
        const balanceInterval = setInterval(() => {
          fetchBalances(unit.value.id);
        }, 5000);

        const transactionsInterval = setInterval(() => {
          fetchTransactions(unit.value.id);
        }, 2000);
        const delinquencyinterval = setInterval(() => {
          fetchDelinquencies(unit.value.id);
        }, 2000);
        ...
        onUnmounted(() => {
          ...
          clearInterval(delinquencyinterval);
        });

        // Pagination for delinquencies and transactions
        const paginatedDelinquencies = computed(() => {
          const startIndex = currentPageDelinquencies.value * itemsPerPage;
          return delinquencies.value.slice(startIndex, startIndex + itemsPerPage);
        });

        const paginatedTransactions = computed(() => {
          const startIndex = currentPageTransactions.value * itemsPerPage;
          return transactions.value.slice(startIndex, startIndex + itemsPerPage);
        });

        const totalPagesDelinquencies = computed(() => Math.ceil(delinquencies.value.length / itemsPerPage));
        const totalPagesTransactions = computed(() => Math.ceil(transactions.value.length / itemsPerPage));

        const nextPageDelinquencies = () => {
          if (currentPageDelinquencies.value < totalPagesDelinquencies.value - 1) {
            currentPageDelinquencies.value++;
          }
        };

        const prevPageDelinquencies = () => {
          if (currentPageDelinquencies.value > 0) {
            currentPageDelinquencies.value--;
          }
        };

        const nextPageTransactions = () => {
          if (currentPageTransactions.value < totalPagesTransactions.value - 1) {
            currentPageTransactions.value++;
          }
        };

        const prevPageTransactions = () => {
          if (currentPageTransactions.value > 0) {
            currentPageTransactions.value--;
          }
        };
      ...
      const goToNextYear = () => {
        ...
        const calendarWeeks = computed(() => {
          ...
          const totalDaysArray = [...offsetDays, ...daysArray];

          const weeks = [];
          while (totalDaysArray.length) {
            weeks.push(totalDaysArray.splice(0, 7));
          }
          return weeks;
        });
    ...
    // Check transactions for the given day
    const hasTollOrDelinquencyPayment = transactions.value.some((transaction) => {
    const transactionDate = new Date(transaction.date);
    
    // Subtract 8 hours (in milliseconds) from the transactionDate
    const adjustedTransactionDate = new Date(transactionDate.getTime() - 8 * 60 * 60 * 1000);

    const formattedTransactionDate = formatDate(adjustedTransactionDate);
    const matchesType = transaction.type === 'toll payment' || transaction.type === 'delinquency payment';
    return formattedTransactionDate === formattedCurrentDate && matchesType;
  });


    // Check delinquencies for the given day
    const hasUnpaidDelinquency = delinquencies.value.some((delinquency) => {
      const delinquencyDate = new Date(delinquency.date_of_payment);
      const formattedDelinquencyDate = formatDate(delinquencyDate);
      const matchesStatus = delinquency.status === 'unpaid';
      return formattedDelinquencyDate === formattedCurrentDate && matchesStatus;
    });

    return {
      'cell-green': hasTollOrDelinquencyPayment,
      'cell-red': hasUnpaidDelinquency,
    };
  };


        const getTransactionClass = (transaction) => {
          const isTollOrDelinquency = transaction.type === 'toll_payment' || transaction.type === 'delinquency_payment';
          const isUnpaidDelinquency = delinquencies.value.some((delinquency) => {
            return delinquency.status === 'unpaid' && delinquency.id === transaction.delinquency_id;
          });

          return {
            green: isTollOrDelinquency,
            red: isUnpaidDelinquency,
          };
        };
        ...
        return {
          unit,
          balance,
          showModalDelinquencies,
          showModalTransactions,
          paginatedDelinquencies,
          paginatedTransactions,
          nextPageDelinquencies,
          prevPageDelinquencies,
          nextPageTransactions,
          prevPageTransactions,
          totalPagesDelinquencies,
          totalPagesTransactions,
          currentPageDelinquencies,
      ...
      generateReceipt(transaction,unit) {
 ...
 // Load the logo from a file
//  const logoqrmcpass = '/C:\Users\Teo\Desktop\letsgoGilas\finalbreathing\CAPS3\QRMCPasss\frontend\src\assets\qrmcpass-logo.png'; // Update this path
//   doc.addImage(logoqrmcpass, 'PNG', 18, 22, 15, 15);


    // Calculate the page width
       // Load the logo (Base64 format)
...
// Insert the logo
doc.addImage(logo, 'PNG', 5, 10, 8, 8); // Adjust the x, y, width, and height as needed
...
const pageWidth = doc.internal.pageSize.getWidth();
    ...
    yPos += 5;
    centerText(branchHeader, yPos, 8);
    yPos += 10;
  ...
  filteredTransactions() {
    if (!this.searchTerm) {
      return this.paginatedTransactions; // If no search term, return all transactions
    }
    ...
    return this.paginatedTransactions.filter(transaction => {
      return (
        new Date(transaction.date_of_payment).toISOString().split('T')[0].toLowerCase().includes(searchTermLower) || // Filter by date
        new Date(transaction.date_of_payment).toLocaleTimeString('en-US', { timeZone: 'GMT' }).toLowerCase().includes(searchTermLower) || // Filter by time
        transaction.amount.toString().toLowerCase().includes(searchTermLower) || // Filter by amount
        transaction.branch.toLowerCase().includes(searchTermLower) || // Filter by branch
        transaction.reference_key.toLowerCase().includes(searchTermLower) || // Filter by reference key
        transaction.teller.toLowerCase().includes(searchTermLower) || // Filter by teller
        transaction.type.toLowerCase().includes(searchTermLower) // Filter by type
      );
    });
  }
}
