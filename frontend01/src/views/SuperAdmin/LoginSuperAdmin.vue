...
export default {
  ...
  methods: {
  login() {
    axios.post('http://127.0.0.1:5000/loginSuperAdmin', this.formData)
      .then(response => {
 ...
 // Store account type separately
        ...
        if (localStorage.getItem('access_token') && localStorage.getItem('accountType') === 'SuperAdmin') {
          
          this.$router.push('/HomeSuperAdmin');
          
          window.location.href = '/HomeSuperAdmin'; // No need for this line
        }
      })
      .catch(error => {
...
created() {
  const isAuthenticated = localStorage.getItem('access_token') && localStorage.getItem('accountType') === 'SuperAdmin';
  if (isAuthenticated) {
    this.$emit('show', true);
